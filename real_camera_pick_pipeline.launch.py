...
def generate_launch_description() -> LaunchDescription:
    ...
    tool_mode_arg = DeclareLaunchArgument("tool_action_mode", default_value="basic_output")
    coord_mode_arg = DeclareLaunchArgument("coord_mode", default_value="0")
    ...
    backend = ExecuteProcess(
        cmd=[
            PYTHON_EXEC,
            os.path.join(here, "real_pick_backend.py"),
            "--ros-args",
            "-p",
            ...
            "-p",
            ["tool_action_mode:=", LaunchConfiguration("tool_mode")],
            "-p",
    ...
    return LaunchDescription(
        [
            ...
            yellow_bin_x_arg,
            yellow_bin_y_arg,
            LogInfo(msg="Starting real camera -> detect -> plan -> real pick pipeline"),
            camera,
            detector,
            planner,
            backend,
